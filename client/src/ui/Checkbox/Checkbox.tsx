	...
	return (
		<div className={twMerge('relative inline-block', disabled && 'pointer-events-none opacity-50', classes)}>
			<input
				...
				checked={checked}
				onChange={onChange}
				className='peer pointer-events-none absolute h-[24px] w-[24px] rounded border-2 border-[#DADADA] focus:ring-transparent'
			/>
			<label htmlFor={name} className={`relative select-none pl-[32px] ${checkedStyle}`}>
				{title}
			</label>
			{checked && <CheckMark className='absolute left-[5px] top-[6px] z-10 h-[11px] w-[14px]' />}
		</div>
	)
}
