...
export const HorizontalTabs: FC<HorizontalTabsPropsType> = ({ tabs, tabMinWidth, classes }) => {
	...
	return (
		<div className={twMerge(classes)}>
			<nav className='flex h-[48px] overflow-x-auto rounded-lg border border-[#DADADA] bg-light-grey p-[3px] xl:inline-flex'>
				{tabs.map((tab, idx) => (
					<button
						...
						type='button'
						className={twMerge(
							`relative h-full w-full select-none whitespace-nowrap rounded-lg border border-transparent px-3 py-2 text-base font-semibold text-grey transition-all hover:cursor-pointer [&:not(:last-child)]:mr-[9px] ${tabBorder}
							xl:w-[${tabMinWidth}px]`,
