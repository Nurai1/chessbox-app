...
=======
	...
	const judgeAssignSuccess = useAppSelector(s => s.competition.setPairJudgeSuccess)
	const submitError = useAppSelector(s => s.competition.setPairJudgeError)
	const dragState = useRef({ dragIndex: 0, hoverIndex: 0 })
>>>>>>> bd3051dbab5fd1401b4f344502cb0ccafb5339a0
...
=======
	...
	const moveItems = useCallback((dragIndex: number, hoverIndex: number) => {
		if (dragState.current.dragIndex !== dragIndex || dragState.current.hoverIndex !== hoverIndex) {
			setGroups(prevGroups => swapArray(prevGroups as CompetitionGroupSchema[], dragIndex, hoverIndex))
			dragState.current = { dragIndex, hoverIndex }
		}
	}, [])
		...
		<main className='container mx-auto grow px-[17px] pt-8 pb-[5.5rem] md:py-9 md:pb-28 xl:pt-14 xl:pl-[7.5rem] xl:pr-[7.5rem]'>
			...
			<BottomFixedContainer classes='xl:pl-[7.5rem] xl:pr-[7.5rem]'>
				<div className='flex flex-wrap gap-2.5'>
					...
					<Button classes='min-w-[8rem] xl:min-w-[15.625rem]' onClick={handleDoneClick} loading={pending}>
						Done
					</Button>
					{submitError && <Alert type='error' subtitle={submitError} />}
				</div>
